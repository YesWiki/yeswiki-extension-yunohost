...
namespace YesWiki\Yunohost\Field;
 ...
 * @Field({"yunohost_user"})
 */
class YunohostUserField extends BazarField
{
    protected $nameField;
    protected $emailField;
    protected $usernameField;
    protected $userMailDomain;
    protected $mailingList;
    protected $autoUpdateMail;
    protected $autoAddToGroup;
    protected $yunohostOptions;
    protected $cmdPrefix;

    protected const FIELD_NAME_FIELD = 1;
    protected const FIELD_EMAIL_FIELD = 2;
    protected const FIELD_USERNAME_FIELD = 3;
    protected const FIELD_USER_MAIL_DOMAIN_FIELD = 4;
    ...
    public function __construct(array $values, ContainerInterface $services)
    {
        ...
        $this->yunohostOptions = $this->getValuesFromConfig();
        $this->nameField = $values[self::FIELD_NAME_FIELD] ?: 'bf_titre';
        ...
        $this->propertyName = 'yunohost_username';
        $this->label = _t('BAZ_USER_FIELD_LABEL');
        $this->maxChars = 60;
    }

    protected function getValuesFromConfig()
    {
    ...
    protected function addYunohostUser($userValues)
    {
        $users = $this->getYunohostUsernames();
        ...
        if ($retval == 1) {
            ...
            if (preg_match('/^This password is among the most used passwords in the world.*$/s', $output[0], $matches, PREG_OFFSET_CAPTURE, 0)) {
                ...
                throw new Exception('Le mot de passe utilisé est trop court, il doit faire au moins 8 caractères.');
            } else {
    ...
    protected function modifyYunohostUser($userId, $userValues)
    {
        #yunohost user update <user> -F '<Prenom Nom>' -d $this->userMailDomain -p '<pass>' --add-mailforward [MAIL ...]]
    }

    protected function deleteYunohostUser($userId)
    {
    ...
    public function formatValuesBeforeSave($entry)
    {
        ...
        if ($value && $this->isUserByName($value)) {
            ...
            $wikiName = strtolower($entry[$this->nameField]);
            ...
            if (!$wiki->IsWikiName($wikiName)) {
                ...
                $wikiName = strtolower(genere_nom_wiki($wikiName, 0));
            }
            ...
            if (!isset($entry[$this->emailField])) {
                throw new Exception("Yunohost user : email not set");
            }
            if (!$isImport) {
                if (!isset($entry['mot_de_passe_repete_wikini'])) {
                    throw new Exception("Yunohost user : password not confirmed second time");
                }
            ...
            try {
                $this->addYunohostUser([
                    'name' => $wikiName,
                    'fullname' => $entry[$this->nameField],
